...
`test_episodes.py::TestCalculateForces::test_expression_is_matched_to_the_right_regulator`
(xfail).
...
half that *is* reachable through the public API -- `test_curves.py::TestBetaCurvesFeedingForceCurves`.
...
`test_curves.py` now covers the reachable failure: a real
...
Tests: `test_chromatin.py::TestSmooth::test_a_single_nan_poisons_the_whole_series`,
...
Tests: `test_chromatin.py::TestProcessSingleWindow::test_malformed_loc_column_is_swallowed_silently`,
...
13. **`get_beta_curves` does not return the links it was asked for.** TFs and
    ...
    (`test_curves.py::TestBetaCurves::test_returns_the_full_tf_by_target_cross_product`)
...
14. **Edge retention uses a row sum.**
    ...
    (`test_episodes.py::TestBuildEpisodeGrn::test_edges_are_retained_by_row_sum_not_by_activity`)
...
17. **`process_dynamics` does not validate `metric`.** Anything other than the
    ...
    `'count'`.
    (`test_chromatin.py::TestProcessDynamics::test_unknown_metric_silently_falls_back_to_counts`)
...
* 17 -- `test_chromatin.py::TestProcessDynamics::test_unknown_metric_silently_falls_back_to_counts`
* 18 -- `test_episodic_units.py::TestFilterChunk::test_a_perfectly_constant_edge_gets_a_degenerate_pvalue_of_zero`
* 19 -- `test_episodes.py::TestEpisodeConstruction::test_episodic_composition_is_an_unimplemented_stub`
* 20 -- `test_chromatin.py::TestScoreVsCountPlot::test_shaded_band_runs_between_score_and_max_of_the_two`
