"""Validation of enriched (FocalFire) links against size-matched random links.

Compares abs-max TF force between the enriched set and a non-enriched null, either
pooled across the trajectory or split by branch-qualified phase. The phase machinery
comes from :mod:`focalfire.temporal._phases`.
"""
...
# bins links to phases via RegulatoryPhases.assign_phases.
from focalfire.temporal._phases import RegulatoryPhases
from focalfire.temporal._waves import TFForceWaves
import matplotlib.pyplot as plt
from focalfire.temporal._phases import _PHASE_ROMAN
...
class TFForceValidation:
    """Validate enriched (FocalFire) links against size-matched random links by
    ...
    * ``'combined'`` -- each enriched link is scored on its strongest lineage
    ...
    def plot(self, figsize=(5, 6), ylabel='Abs max TF force',
        ...
        if self.result_ is None:
            self.run()
        return plot_force_validation_boxes(self.result_, figsize, ylabel, colors)
    ...
    def plot_multi(df, group_order, group_labels=None, figsize=(7, 6),
        ...
        """
        return plot_force_validation_multi(df, group_order, group_labels,
                                           figsize, ylabel, colors)
    ...
    def plot_multi_by_phase(df, branch, group_order, group_labels=None,
        ...
        """
        return plot_force_validation_by_phase(df, branch, group_order, group_labels,
                                              figsize, ylabel, colors, xlabel)
    ...
    class PhaseSplitted:
        ...
        def plot(self, figsize=(9, 6), ylabel='Abs max TF force',
            ...
            if self.result_ is None:
                self.run()
            return plot_force_validation_phase_cells(self.result_, figsize, ylabel, colors)


# ---------------------------------------------------------------------------
# Figures: enriched vs random force boxes
# ---------------------------------------------------------------------------
...
def plot_force_validation_boxes(df, figsize=(5, 6), ylabel='Abs max TF force',
    ...
    groups = [('enriched', 'Enriched (FocalFire)'),
