...
class DatasetPaths:
    ...
    def find(
        ...
        env: str = "FOCALFIRE_DATASETS",
    ) -> "DatasetPaths":
        """Locate the dataset YAML without hard-coding where the kernel started.

        Resolution order:

        1. the environment variable ``env`` (default ``FOCALFIRE_DATASETS``), when
        ...
        ``FOCALFIRE_DATASETS`` once instead of hard-coding an absolute path in a cell.
        """
        import os
